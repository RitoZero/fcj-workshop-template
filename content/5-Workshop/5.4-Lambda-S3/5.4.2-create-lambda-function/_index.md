...
## 6. Save a deployment checkpoint

Verify:

- the latest ZIP is uploaded;
- `filters.py` supports `expression`;
- the handler is `lambda_function.lambda_handler`;
- all six environment variables exist;
- the execution role has both the custom policy and
  `AWSLambdaBasicExecutionRole`;
- no real JWT, client secret, access key, or presigned URL appears in the code.

Do not use a direct Lambda console test as the final API test. The router reads
JWT claims and `routeKey` values generated by API Gateway, so a manually written
Lambda event must reproduce that structure. Complete the API integration first.
